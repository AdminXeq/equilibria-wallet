<template>
  <div class="settings-general">
        ...
        <q-expansion-item label="Advanced Options" header-class="q-mt-sm non-selectable row reverse advanced-options-label">
          <div class="row justify-between q-mb-md">
            <div>
          ...
          <p v-if="config_daemon.type == 'local_remote'">
            Get started quickly with this default option. Wallet will download the full blockchain,
            but use a remote node while syncing.
          </p>
          <p v-if="config_daemon.type == 'local'">
            Full security, wallet will download the full blockchain.
            You will not be able to transact until sync is completed.
          </p>
          <p v-if="is_remote">
            Less security, wallet will connect to a remote node to make all transactions.
          </p>
          <div class="col q-mt-md pt-sm">
            ...
            <tritonField label="Wallet Storage Path" disable-hover>
              ...
              <input id="walletPath" ref="fileInputWallet" type="file" webkitdirectory directory hidden @change="setWalletDataPath" />
          ...
          <div class="row pl-sm q-mt-sm">
            <tritonField class="col-6" label="Daemon Log Level" :disable="is_remote">
              <q-input v-model="config_daemon.log_level"
                placeholder="toString(daemon_defaults.log_level)"
                :disable="is_remote" :dark="theme=='dark'" type="number"
                :decimals="0" :step="1" min="0" max="4"
                borderless dense
              />
            </tritonField>
          ...
          <div class="row pl-sm q-mt-md">
            ...
            <tritonField class="col-3" label="Limit Upload Rate" :disable="is_remote">
              <q-input v-model="config_daemon.limit_rate_up"
                ...
                :disable="is_remote" :dark="theme=='dark'" type="number" suffix="Kb/s"
                :decimals="0" :step="1" min="-1" max="65535"
                borderless dense
              />
            </tritonField>
          ...
          <div class="row pl-sm q-mt-md">
            <tritonField class="col-3" label="Daemon P2P Port" :disable="is_remote">
              <q-input v-model="config_daemon.p2p_bind_port"
                ...
                :disable="is_remote" :dark="theme=='dark'" float- type="number"
                :decimals="0" :step="1" min="1024" max="65535"
                borderless dense
              />
            </tritonField>
            ...
            <tritonField class="col-3" label="Internal Wallet Port">
              <q-input v-model="config.app.ws_bind_port"
                ...
                :dark="theme=='dark'" float- type="number"
                :decimals="0" :step="1" min="1024" max="65535"
                borderless dense
              />
            </tritonField>
            <tritonField class="col-3" label="Wallet RPC Port" :disable="is_remote">
              <q-input v-model="config.wallet.rpc_bind_port"
                placeholder="asString(defaults.wallet.rpc_bind_port)"
                :disable="is_remote" :dark="theme=='dark'" float- type="number"
                :decimals="0" :step="1" min="1024" max="65535"
                borderless dense
              />
          ...
          <tritonField helper="Choose a network" label="Network" class="network-group-field">
            <q-option-group v-model="config.app.net_type" type="radio"
              :options="[
